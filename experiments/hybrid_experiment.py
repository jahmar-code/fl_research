...
from privacy.hybrid import HybridFederatedClient, HybridFederatedServer
...
def run_hybrid_experiment(num_clients=10, num_rounds=20, iid=True, alpha=0.5, local_epochs=1, batch_size=64,
                         # DP parameters
    ...
    """
    Run a federated learning experiment with both differential privacy and homomorphic encryption.
    ...
    logging.info("Starting Federated Learning with Hybrid (HE+DP) Privacy experiment")
    ...
    # Initialize server with HE+DP capabilities
    server = HybridFederatedServer(global_model, test_loader)
    
    # Initialize clients with HE+DP capabilities
    clients = []
    for client_id in range(num_clients):
        client = HybridFederatedClient(
            client_id=client_id,
            model=SmallCNN(),
    ...
    history = []
    epsilon_history = []
    noise_multiplier_history = []  # Add tracking for noise multiplier
    ...
    for round_idx in range(num_rounds):
        ...
        try:
            # Perform one round of federated averaging with hybrid privacy
            ...
            if 'avg_epsilon' in round_metrics:
                ...
                if noise_count > 0:
                    ...
                    noise_multiplier_history.append(noise_multiplier if noise_multiplier is not None else 1.0)
                    logging.info(f"Round {round_idx+1} completed. "
                                f"Accuracy: {round_metrics['accuracy']:.2f}%, "
                                f"Loss: {round_metrics['loss']:.4f}, "
                                f"Epsilon: {epsilon:.4f}, "
    ...
    # Plot results
    plot_dp_results(results, "FL with HE+DP")
    
    return results

if __name__ == "__main__":
    run_hybrid_experiment(num_rounds=10)  # Fewer rounds for hybrid due to computational overhead 