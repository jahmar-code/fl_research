...
from privacy.he import HEFederatedClient

class HybridFederatedClient(FederatedClient):
    """Client with both differential privacy and homomorphic encryption capabilities."""
    ...
    def __init__(self, client_id, model, dataloader, criterion, optimizer_class, optimizer_args,
        ...
        """
        Initialize a federated learning client with both DP and HE.
        ...
        self.noise_multiplier = noise_multiplier if noise_multiplier is not None else 0.8  # Default to moderate noise
        
        # Create privacy engine
        self.privacy_engine = PrivacyEngine()
        
        # Use more optimized DP settings
    ...
    def train(self):
        ...
        noise_mult = self.noise_multiplier  # Use the actual noise multiplier value
    ...
    def encrypt_model_update(self, update):
        ...
        for param in update:
            ...
            # Note: Explicitly use float64 for HE operations to avoid buffer dtype mismatch
            ...
            for i in range(0, len(quantized), max_chunk_size):
                ...
                try:
                    ...
                    print(f"Encryption error with chunk of shape {chunk.shape} and dtype {chunk.dtype}: {e}")
                    # Try again with proper dtype
    ...
    def get_model_update(self):
        ...
        Apply DP during training, then encrypt the update with HE before sending.
        ...
        for encrypted_param in encrypted_aggregated_update:
            ...
            for chunk in chunks:
                try:
                    ...
                    # This is a fallback that shouldn't be needed if encryption is fixed
            ...
            # Note: Convert to float32 for PyTorch compatibility while preserving data
    ...
    def set_model_parameters(self, parameters):
        ...
        if isinstance(parameters, dict) and 'encrypted' in parameters:
            ...
            try:
                ...
                # Fall back to using original parameters
...
class HybridFederatedServer(FederatedServer):
    """Server with both differential privacy and homomorphic encryption capabilities."""
    
    def __init__(self, global_model, test_loader=None):
        """
        Initialize a federated learning server with both DP and HE capabilities.
    ...
    def aggregate_encrypted(self, client_updates):
        ...
        # Extract encrypted weights - use only the first client's updates as a simplification
        # In a real system, proper secure aggregation would be implemented
        first_client_encrypted = client_updates[0]['encrypted']
        
        # Simply return the first client's encrypted updates for now
        # This is a simplification to demonstrate the hybrid approach concept
        # A real system would properly aggregate the encrypted updates
        return {'encrypted': first_client_encrypted}
    
    def federated_averaging(self, selected_clients):
        """
        Perform one round of federated averaging with encrypted updates.
        ...
        try:
            ...
            # In a real-world scenario, you might use secure multi-party computation or a trusted third party
            ...
            # Log the error and return basic metrics
            print(f"Error in federated averaging: {str(e)}")
            computation_time = time.time() - start_time
